...
@bind RNA_parameters PlutoUI.combine() do Child
	...
	md"""
	  K $(
		Child(Slider(0.065:0.01:1))
	) (mM)  W1_new $(
		Child(Slider(0.27:0.001:0.4))
	)  W2_new $(
		Child(Slider(100:1:110))
	)  
	"""
end
...
begin
	...
	for i in eachindex(I)
		...
		f0_new = 1.0 # state 0 
		f1_new = 1.0 # state 1
	    f2_new = ((I[i]/K)^(n_new))/(1+I[i]/K)^(n_new)# state 2
...
md"""
##### calculate $\kappa_{x}$(G...) and $m*$, try to find the proper values match the $m*$ to the converted data from copy numbers that are given in table 2 when with the same concentration I (set Kd = 430 )
"""
...
begin
	...
	m = r_x/(θₓ+μ)*u * Vc / mc * 10^(-15)
	
	# show - 
	with_terminal() do		
	println("m = $(m[1]),$(m[5]),$(m[50]),$(m[120]),$(m[530]),$(m[2160]),$(m[10000])")
	end
end

# ╔═╡ 0187b3e2-d335-4a9b-99f6-db66da25138f
md""" 
...
begin
   # IPTG concentration -
	...
	m_array = [m[1];m[5];m[50];m[120];m[530];m[2160];m[10000];];
	...
	plot!(concI,m_array,yerror=(low_array_new,high_array_new),seriestype = :scatter, legend = false)
	xlabel!("IPTG concentration (mM)",fontsize=18)
	ylabel!("cell mass specific concentration (nmol/gDW)",fontsize=18)
end
